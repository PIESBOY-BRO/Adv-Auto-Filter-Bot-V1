    ...
    if filters:
        for filter in filters: # iterating through each files
            ...
            if file_size < 1024:
                ...
                file_size = f"[{str(round(file_size/𝚜𝚞𝚋𝚝𝚒𝚝𝚕𝚎))} 𝚜𝚞𝚋𝚝𝚒𝚝𝚕𝚎] "
    ...
    if len(results) == 0: # double check
        ...
        try:
            await bot.send_message(
                ...
                text=f"Found {(len_results)} Results For Your request                  സിനിമ ലഭിക്കാൻ താഴെ കാണുന്ന ലിംഗുകളിൽ ക്ലിക്ചെയ്തശേഷം  ⟮ 𝐒𝐓𝐀𝐑𝐓 ⟯ ബട്ടനിൽ ക്ലിക്ചെയ്യുക: <code>{query}</code>",
